...
private final class StorageMock: FavoritesStorageProtocol {
    var ids: Set<Int> = []
...
final class FavoritesStoreTests: XCTestCase {
    func testInitialLoadPopulatesIds() {
        ...
        mock.ids = [1,2]
        let store = FavoritesStore(storage: mock)
        let exp = expectation(description: "loaded")
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
            XCTAssertEqual(store.getFavoriteMovieIds(), [1,2])
            exp.fulfill()
        }
    ...
    func testToggleFavoriteAddsOptimisticallyAndRollsBackOnFailure() {
        ...
        XCTAssertTrue(store.getFavoriteMovieIds().contains(10))
